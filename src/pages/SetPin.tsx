...
export default function SetPin() {
  ...
  const endpointsFunctions = {
    verifyToken: async (userId: string, token: string) =>
      axios.post(`${import.meta.env.VITE_API_BASE_URL || 'https://api.kuditime.com/api'}/auth/verify-otp`, {
        userId: userId,
        otp: token,
        purpose: "RESET_PIN"
      }),
    setNewPin: async (userId: string, token: string, pin: string) =>
      axios.post(`${import.meta.env.VITE_API_BASE_URL || 'https://api.kuditime.com/api'}/auth/set-pin`, {
        userId: userId,
        token: token,
        pin,
      }),
  ...
  const handleSetNewPin = async (userId: string, token: string, pin: string) => {
    setLoading(true);
    try {
      await endpointsFunctions.setNewPin(userId, token, pin);
      setSuccess(true);
      setTokenIsInvalid(false);
    } catch (error) {
      setSuccess(false);
  ...
  const onSubmit = (data: z.infer<typeof validationSchema>) => {
    ...
    const userId = searchParams.get("userId");
    if (userId && token) {
      handleSetNewPin(userId, token, data.pin);
    }
  };
          ...
          <div className="bg-green-100 border-l-4 border-green-500 rounded text-green-900 px-4 py-3 shadow-md">
            <div className="flex">
              ...
              <div>
                ...
                <p className="text-sm">
                  Your PIN has been set successfully, you can now use your new PIN on your mobile app.
                </p>
              </div>
            </div>
          </div>
